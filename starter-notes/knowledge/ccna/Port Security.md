...
Port Security limits the number of unique [[MAC Address]] a switch can learn on a single port and defines actions when that limit is exceeded. By default, switches can learn thousands of MAC addresses per port (limited only by the MAC address table size). This is a vulnerability.
