...
# Create YOUR_LAB service accounts in a dedicated OU.
...
$SvcOU   = "OU=LAB-ServiceAccounts,$Domain"
...
if (-not (Get-ADOrganizationalUnit -Filter "DistinguishedName -eq '$SvcOU'" -ErrorAction SilentlyContinue)) {
    New-ADOrganizationalUnit -Name "LAB-ServiceAccounts" -Path $Domain
    Write-Host "[created OU] LAB-ServiceAccounts" -ForegroundColor Cyan
}
...
Write-Host "`nCreating service accounts in OU: LAB-ServiceAccounts`n"
...
foreach ($Svc in $ServiceAccounts) {
    ...
    New-ADUser `
        ...
        -UserPrincipalName     "$($Svc.Name)@yourdomain.local" `
    ...
    Write-Host "  UPN:       $($Svc.Name)@yourdomain.local"
