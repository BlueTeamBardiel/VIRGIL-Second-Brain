...
Ethernet switching operates at [[Layer 2 (Data Link Layer)]] and is responsible for delivering frames between devices on the same local area network ([[LAN]]). Unlike routers (which work at Layer 3), switches make forwarding decisions based on **MAC addresses**, not IP addresses. This chapter covers the mechanics of frame forwarding, MAC address learning, and the critical protocols that keep switching networks functional.
