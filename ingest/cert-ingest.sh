...
TODAY = os.environ["INGEST_TODAY"]

temp_file = os.environ.get("VIRGIL_INGEST_TEMP", "")
if temp_file and Path(temp_file).exists():
    raw_text = Path(temp_file).read_text(encoding='utf-8', errors='replace')[:40000]
else:
    print("[cert-ingest] ERROR: temp file not found")
    sys.exit(1)
...
index_lines = [
    ...
    f"> Source: {os.path.basename(os.environ.get('INGEST_SOURCE', 'unknown'))}",
    f"",
    f"## Chapters",
    f"",
]
